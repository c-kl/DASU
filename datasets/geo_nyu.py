...
class Geo_NYUDataset(Dataset):
    def __init__(self, root='/data3/tang/nyu_labeled', split='train', scale=8, augment=True, downsample='bicubic',
                 pre_upsample=False, to_pixel=False, sample_q=None, input_size=None, noisy=False, if_AR=True):
        super().__init__()
        self.root = root
        self.split = split
    ...
    def __getitem__(self, idx):
        ...
        depth_HF_file = os.path.join(self.root, 'HF_NPY', f'{idx}.npy')

        image = cv2.imread(image_file)  # [H, W, 3]
        depth_hr = np.load(depth_file)  # [H, W]
        depth_HF_hr = np.load(depth_HF_file)  # [H, W]
        # crop after rescale
        if self.input_size is not None:
            ...
            depth_HF_hr = depth_HF_hr[x0:x0 + self.input_size, y0:y0 + self.input_size]
        ...
        if self.downsample == 'bicubic':
            ...
            image_lr = np.array(Image.fromarray(image).resize((int(w // self.scale), int(h // self.scale)),
                                                              Image.BICUBIC))  # bicubic, RMSE=7.13
            depth_HF_lr = np.array(Image.fromarray(depth_HF_hr).resize((int(w // self.scale), int(h // self.scale)),
                                                                 Image.BICUBIC))  # bicubic, RMSE=7.13
            ...
            depth_lr = depth_hr[(self.scale - 1)::self.scale,
                       (self.scale - 1)::self.scale]  # right-bottom, RMSE=14.22, finally reproduced it...
            image_lr = image[(self.scale - 1)::self.scale,
                       (self.scale - 1)::self.scale]  # right-bottom, RMSE=14.22, finally reproduced it...
        elif self.downsample == 'nearest-center':
            depth_lr = np.array(Image.fromarray(depth_hr).resize((w // self.scale, h // self.scale),
                                                                 Image.NEAREST))  # center (if even, prefer right-bottom), RMSE=8.21
            image_lr = np.array(Image.fromarray(image).resize((w // self.scale, h // self.scale),
                                                              Image.NEAREST))  # center (if even, prefer right-bottom), RMSE=8.21
        ...
        depth_HF_hr_min = depth_HF_hr.min()
        depth_HF_hr_max = depth_HF_hr.max()
        depth_HF_hr = (depth_HF_hr - depth_HF_hr_min) / (depth_HF_hr_max - depth_HF_hr_min)
        depth_HF_lr = (depth_HF_lr - depth_HF_hr_min) / (depth_HF_hr_max - depth_HF_hr_min)
        ...
        depth_HF_lr_up = np.array(Image.fromarray(depth_HF_lr).resize((w, h), Image.BICUBIC))

        if self.pre_upsample:
            depth_lr = depth_lr_up
            depth_HF_lr = depth_HF_lr_up
        ...
        depth_HF_hr = torch.from_numpy(depth_HF_hr).unsqueeze(0).float()
        depth_HF_lr = torch.from_numpy(depth_HF_lr).unsqueeze(0).float()
        depth_HF_lr_up = torch.from_numpy(depth_HF_lr_up).unsqueeze(0).float()
        ...
        if self.augment:
            ...
            depth_HF_hr = augment(depth_HF_hr)
            depth_HF_lr = augment(depth_HF_lr)
            depth_HF_lr_up = augment(depth_HF_lr_up)
        ...
        depth_HF_hr = depth_HF_hr.contiguous()
        depth_HF_lr = depth_HF_lr.contiguous()
        depth_HF_lr_up = depth_HF_lr_up.contiguous()

        depth_LF_hr = depth_hr - depth_HF_hr
        depth_LF_lr = depth_lr - depth_HF_lr
        depth_LF_lr_up = depth_lr_up - depth_HF_lr_up
...
#Struct=HF
            return {
                ...
                'depth_HF_hr': depth_HF_hr,
                'depth_HF_lr': depth_HF_lr,
                'depth_HF_lr_up': depth_HF_lr_up,
                'depth_LF_hr': depth_LF_hr,
                'depth_LF_lr': depth_LF_lr,
                'depth_LF_lr_up': depth_LF_lr_up,
                'hr_coord': hr_coord,
                'min': depth_min * 100,
                'max': depth_max * 100,
                'idx': idx,
