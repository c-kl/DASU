...
class Trainer(object):
    ...
    def train_step(self, data):
        # print('--------------------------------------------------------')
        gt, depth_LF_hr,depth_HF_hr= data['hr_depth'],data['depth_LF_hr'],data['depth_HF_hr']
        #depth_LF_hr = data['depth_LF_hr']
        #depth_HF_hr = data['depth_HF_hr']
        # a = time.time()
        pred ,pred_LF,pred_HF= self.model(data)
        # b = time.time()
        # self.sum_t += (b-a)
        # print("loss--", gt.shape, pred.shape)
        #d1 = nn.Parameter(torch.Tensor([0.5])).cuda()
        #d2 = nn.Parameter(torch.Tensor([0.5])).cuda()
        loss1 = self.objective(pred, gt)
        loss2 = self.objective(pred_LF, depth_LF_hr)
        loss3 = self.objective(pred_HF, depth_HF_hr)
        ...
        # pred_LF = pred_LF * (data['max'] - data['min']) + data['min']
        # gt_LF = depth_LF_hr * (data['max'] - data['min']) + data['min']
        # pred_detail = pred_detail * (data['max'] - data['min']) + data['min']
        # gt_HF = depth_HF_hr * (data['max'] - data['min']) + data['min']
    ...
    def test_step(self, data):
        ...
        gt = data['hr_depth']
        input = data["lr_depth"]
        depth_LF_hr, depth_HF_hr =  data['depth_LF_hr'], data['depth_HF_hr']

        a = time.time()
        pred,pred_LF,pred_detail = self.model(data)
        b = time.time()
        dtime = b-a
        pred = pred * (data['max'] - data['min']) + data['min']
        gt = gt * (data['max'] - data['min']) + data['min']
        ...
        return pred, gt, dtime, input,pred_LF,gt_LF,pred_detail,gt_HF
    ...
    def test(self, loader, save_path=None):
        ...
        with torch.no_grad():
            ...
            for data in loader:
                ...
                preds, gt, dtime, input ,pred_LF,gt_LF,pred_detail,gt_HF= self.test_step(data)
                all_time += dtime
                ...
                for b in range(preds.shape[0]):
                    ...
                    plt.imsave(os.path.join(error_path,  self.name+'error'+idx+'.png'), error,vmin=5,vmax=25,  cmap="jet")
                    # import cv2
                    # cv2.imwrite(os.path.join(save_path, self.name + idx + '.png'), pred)
                    # cv2.imwrite(os.path.join(gt_path, self.name + idx + '.png'), gt)
